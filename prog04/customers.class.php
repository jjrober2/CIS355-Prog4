...
class Customer { 
    ...
    function list_records() {
        ...
        echo "
            ...
            <body>
                <a href='https://github.com/ajsliter/Prog04' target='_blank'>Github</a><br />
                <div class='container'>
                    <p class='row'>
                        <h3>$this->title" . "s" . "</h3>
                    </p>
        ...
        foreach ($pdo->query($sql) as $row) {
			...
			//Code for Avatar Views
			if(empty($row["filestore"])) {
				//Option 1: No File in Database
				echo "<td><a class='btn btn-info' href='upload_avatar.html?id=" . $row["id"] . "'>Upload Avatar</a></td>";
			} else {
				//Option 2: File Exists in Database
				echo "<td> <img width='100' src='data:image/jpeg;base64, " . base64_encode( $row["filestore"] ) . "'/> <br> <a type='file' class='btn btn-warning' href='upload_avatar.html?id=" . $row["id"] . "'>Replace Avatar</a> </td>";
			}
            echo "<td> <a href='". $row["filelinkedpath"] . "' target='_blank'>". $row[filelinkedpath] . "</a></td>";
            echo "<td width=250>";
		...
		<a href='customer_img' target='_blank'>User Images</a>
		<br>
		<a href='img/Prog03UMLDiag.png' target='_blank'>UML Class Diagram (Unchanged from Prog 03)</a>
		<br>
		<a href = 'login2.html' target='_blank'>User Flow Diagram</a>
